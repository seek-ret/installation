...
WORKSPACE=$(/opt/elasticbeanstalk/bin/get-config environment -k WORKSPACE)
...
sudo docker run -d --rm --net host -e ACCESS_KEY="${ACCESS_KEY}" -e SECRET_ACCESS_KEY="${SECRET_ACCESS_KEY}" \
  -e WORKSPACE="${WORKSPACE}" -e BUCKET_NAME="${BUCKET_NAME}" -e STORAGE_PROVIDER="${STORAGE_PROVIDER}" \
